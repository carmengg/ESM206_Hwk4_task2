...
Yearly counts of juvenile snowshoe hares are analyzed. Sample site is not included as a variable here, though it would be it would be interesting to see if juvenile hare counts are changing equally in all sample sites. 
...
# --- Summary statistics
year_median <- median(per_year_juveniles$n)
year_mean <- mean(per_year_juveniles$n)
year_min <- min(per_year_juveniles$n)
year_max <- max(per_year_juveniles$n)
```

**Figure 1.** Total juvenile snowshoe hare observations (n) during each year of the study (1999-2012). 

No observations of juvenile snowshoe hares were made during 2002 and 2009, while the highest count was registered in 1999 (the first year of the study) with `r year_max` observations. On average `r year_mean` juvenile hare trappings were made from 1999 to 2012 and the median was `r year_median`. In figure one we can see a  diminishing trend in the number of juvenile hare trappings. The two exceptions are 2005, 2008 and 2011. In 2005 juvenile hare counts were 
